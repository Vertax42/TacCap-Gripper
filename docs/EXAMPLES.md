...
| `force_position_console.py`      | Interactive test for the SDK `ForcePositionController`: `t` enters an exact normalized target and optional hold torque, `g` repeats it, `o` performs a bounded full release, and `h` holds the current position. Separates the device/motion limit (up to 6 Nm) from the software force-hold ceiling (up to 1.8 Nm); `--set-device-limit` writes the MCU boot value and exits so the unit can be power-cycled. |
